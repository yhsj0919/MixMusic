    ...
    rec: {
        ...
        mv: function mvRec() {
            ...
            const params = {
                data: JSON.stringify({
                    ...
                    mv: {
                        ...
                        method: "GetNewMv",
                        param: {"style": 0, "tag": 0, "size": 10, "start": 0},
                    }
                })
            }
            ...
            return axios.get('https://u.y.qq.com/cgi-bin/musicu.fcg', {
                ...
                const result = respData["mv"]["data"]["list"]
                ...
                data: JSON.stringify({
                    ...
                    singerList: {
                        ...
                        param: {
                            area: type["area"] ?? -100,
                            sex: type["sex"] ?? -100,
                            genre: type["genre"] ?? -100,
                            index: type["index"] ?? -100,
                            sin: sin,
    ...
    mv: {
        type: function artistType() {
            ...
            return axios.get('https://u.y.qq.com/cgi-bin/musicu.fcg', {
                ...
                const result = respData["tag"]["data"]
                ...
                newArray.push({
                    ...
                    package: 'xyz.yhsj.qq',
                    subType: [
                        {package: 'xyz.yhsj.qq', id: 1, title: "最新",},
                        {package: 'xyz.yhsj.qq', id: 0, title: "最热",}
                    ],
                },)
                return {
                    code: 200,
        ...
        list: function (type, page = 0, size = 20) {

            const sin = parseInt(page) * size;
            ...
            const params = {
                data: JSON.stringify({
                    ...
                    list: {
                        ...
                        param: {
                            version_id: type["version"] ?? 7,
                            area_id: type["area"] ?? 15,
                            order: type["order"] ?? 1,
                            start: sin,
                            size: size,
                        }
            ...
            return axios.get('https://u.y.qq.com/cgi-bin/musicu.fcg', {
                ...
                const result = respData["list"]["data"]

                const newArray = result["list"].map(function (element) {
                    return {
                        package: 'xyz.yhsj.qq',
                        id: element["vid"],
                        pic: element["picurl"],
        ...
        info: function (mv, page = 0, size = 20) {
            ...
            return axios.get('https://u.y.qq.com/cgi-bin/musicu.fcg', {
                ...
                const info = respData["info"]["data"][mv['id']];
                mv.desc = info["desc"];

                const urls = respData["mvUrl"]["data"][mv['id']]["mp4"];

                const qualities = urls
                    .filter(item => item["freeflow_url"].length > 0)  // 筛选 freeflow_url 不为空的条目
                    .map(item => {
                        return {
                            package: 'xyz.yhsj.qq',
                            id: mv["id"],
                            title: item["filetype"] === 10 ? "标清" : item["filetype"] === 20 ? "高清" : item["filetype"] === 30 ? "超清" : item["filetype"] === 40 ? "全高清" : "其他",
                            quality: item["filetype"] === 10 ? 360 : item["filetype"] === 20 ? 480 : item["filetype"] === 30 ? 720 : item["filetype"] === 40 ? 1080 : 360,
                            size: item["fileSize"],
                ...
                if (qualities.length > 0) {
                    const temp = qualities[qualities.length - 1];
                    console.log(JSON.stringify(temp))
                    mv.url = temp["url"];
                }

                mv.quality = qualities;
                return {
                    code: 200,
                    msg: '操作成功',
                    data: mv,
