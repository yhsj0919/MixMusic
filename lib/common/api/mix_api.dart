...
import 'package:mix_music/common/entity/app_resp_entity.dart';
...
import 'package:mix_music/common/entity/mix_user.dart';
...
class MixApi extends MusicApi {
  ...
  Future<void> _init() async {
    ...
    await current?.injectMethod("setStorage", (args) async {
      String key = args[0];
      String value = args[1];
      return await AppDB.setString("${plugins?.package}_$key", value.toString());
    });
    await current?.injectMethod("getStorage", (args) {
      String key = args[0];
      return AppDB.getString("${plugins?.package}_$key") ?? "";
    });
  ...
  @override
  Future<MixLrc> lrc(MixSong song) {
    var mySong = JsonMapper.toMap(song);
    return invokeMethod(method: "music.url.lrc", params: [mySong]).then((value) {
      AppRespEntity<MixLrc> data = AppRespEntity.fromJson(value);
      if (data.code == 200) {
        return Future(() => data.data!);
      } else {
        return Future.error(data.msg ?? "操作失败");
