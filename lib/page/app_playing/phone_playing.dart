...
class _PhonePlayingState extends State<PhonePlaying> {
  ...
  Widget build(BuildContext context) {
    double statusBarHeight = max(MediaQuery.of(context).padding.top, 8);
    double bottom = max(MediaQuery.of(context).padding.bottom, 8);
    // 获取状态栏的高度
    final double width = MediaQuery.of(context).size.width;

    return Scaffold(
      appBar: context.isLandscape ? null : AppBar(),
      body: Stack(
        children: [
          Obx(
            () => Container(
      ...
      () => Container(
        ...
        child: Hero(
          ...
          child: Stack(
            ...
            children: [
              AppImage(radius: 12, width: 250, height: 250, url: music.currentMusic.value?.pic?.toString() ?? ""),
              Obx(
                () => Container(
                  height: 30,
