...
class DesktopArtistDetailAlbum extends StatefulWidget {
  const DesktopArtistDetailAlbum({super.key, required this.artist});

  final MixArtist artist;

  @override
  State<DesktopArtistDetailAlbum> createState() => _DesktopArtistDetailAlbumState();
}

class _DesktopArtistDetailAlbumState extends State<DesktopArtistDetailAlbum> with AutomaticKeepAliveClientMixin {
  ...
  void artistSong({required MixArtist artist, int page = 0}) {
    ApiFactory.api(package: widget.artist.package)
        ?.artistAlbum(artist: artist, page: page, size: 20)
        .then((value) {
