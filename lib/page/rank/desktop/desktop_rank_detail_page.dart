...
class DesktopRankDetailPage extends StatefulWidget {
  const DesktopRankDetailPage({super.key, required this.item});

  final MixRank item;

  @override
  State<DesktopRankDetailPage> createState() => _DesktopRankDetailPageState();
}

class _DesktopRankDetailPageState extends State<DesktopRankDetailPage> {
                              ...
                              children: [
                                AspectRatio(
                                  ...
                                  child: Hero(
                                    ...
                                    child: AppImage(url: rank.value?.pic ?? "", width: 200, height: 200),
                                  ),
                                ),
                                AspectRatio(
                                  aspectRatio: 1,
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (offset < 2)
                                Obx(() => Text("${rank.value?.songCount ?? "0"}首歌", style: FluentTheme.of(context).typography.bodyStrong?.copyWith(fontWeight: FontWeight.normal))),
                              Row(
                                spacing: 16,
                                children: [
                                  Button(
  ...
  void getRankInfo({int page = 0}) {
    ApiFactory.api(package: rank.value?.package ?? "")
        ?.rankInfo(rank: rank.value!, page: page, size: 20)
        .then((value) {
