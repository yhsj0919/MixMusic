...
class DesktopSearchArtistPage extends StatefulWidget {
  const DesktopSearchArtistPage({super.key, required this.plugin, required this.controller});
  ...
  @override
  SearchTabPageState<DesktopSearchArtistPage> createState() => _DesktopSearchArtistPageState();
}

class _DesktopSearchArtistPageState extends SearchTabPageState<DesktopSearchArtistPage> with AutomaticKeepAliveClientMixin {
  late EasyRefreshController refreshController;
  MusicController music = Get.put(MusicController());

  Rxn<PageEntity> pageEntity = Rxn();
  RxList<MixArtist> songList = RxList();
        ...
        child: dataLoad.value
            ...
            : PageGridView(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return Center(
                    child: HomeArtistItem(
                      onTap: () {
                        context.go(Routes.artistDetail, extra: item);
                      },
                      item: item,
                    ),
                  );
                ...
                gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
                  maxCrossAxisExtent: 160, // 每个元素的最大宽度
                  crossAxisSpacing: 4,
                  mainAxisSpacing: 4,
  ...
  void searchSong({required String? keyword, int page = 0, int size = 20}) {
    ...
    ApiFactory.api(package: widget.plugin.package!)
        ?.searchArtist(keyword: keyword ?? "", page: page, size: size)
        .then((value) {
