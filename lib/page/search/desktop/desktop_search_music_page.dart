...
class DesktopSearchMusicPage extends StatefulWidget {
  const DesktopSearchMusicPage({super.key, required this.plugin, required this.controller});
  ...
  @override
  SearchTabPageState<DesktopSearchMusicPage> createState() => _DesktopSearchMusicPageState();
}

class _DesktopSearchMusicPageState extends SearchTabPageState<DesktopSearchMusicPage> with AutomaticKeepAliveClientMixin {
  ...
  void searchSong({required String? keyword, int page = 0, int size = 20}) {
    ...
    ApiFactory.api(package: widget.plugin.package!)
        ?.searchMusic(keyword: keyword ?? "", page: page, size: size)
        .then((value) {
