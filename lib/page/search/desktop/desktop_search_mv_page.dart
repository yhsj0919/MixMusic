...
class DesktopSearchMvPage extends StatefulWidget {
  const DesktopSearchMvPage({super.key, required this.plugin, required this.controller});
  ...
  @override
  SearchTabPageState<DesktopSearchMvPage> createState() => _DesktopSearchMvPageState();
}

class _DesktopSearchMvPageState extends SearchTabPageState<DesktopSearchMvPage> with AutomaticKeepAliveClientMixin {
  ...
  void searchSong({required String? keyword, int page = 0, int size = 20}) {
    ...
    ApiFactory.api(package: widget.plugin.package!)
        ?.searchMv(keyword: keyword ?? "", page: page, size: size)
        .then((value) {
