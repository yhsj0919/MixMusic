...
class DesktopSearchPlayListPage extends StatefulWidget {
  const DesktopSearchPlayListPage({super.key, required this.plugin, required this.controller});
  ...
  @override
  SearchTabPageState<DesktopSearchPlayListPage> createState() => _DesktopSearchPlayListPageState();
}

class _DesktopSearchPlayListPageState extends SearchTabPageState<DesktopSearchPlayListPage> with AutomaticKeepAliveClientMixin {
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return Center(
              child: HomePlayListItem(
                item: item,
                onTap: () {
                  context.go(Routes.playListDetail, extra: item);
                },
              ),
            );
          },
  ...
  void searchSong({required String? keyword, int page = 0, int size = 20}) {
    ...
    ApiFactory.api(package: widget.plugin.package!)
        ?.searchPlayList(keyword: keyword ?? "", page: page, size: size)
        .then((value) {
