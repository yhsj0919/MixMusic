...
import 'package:mix_music/page/search/mobile/search_album_page.dart';
import 'package:mix_music/page/search/mobile/search_artist_page.dart';
import 'package:mix_music/page/search/mobile/search_music_page.dart';
...
class _SearchPageState extends State<SearchPage> with TickerProviderStateMixin {
  ...
  Future<void> getSearchHistory() async {
    searchList.value = await AppDB.getStringList(Constant.KEY_APP_HISTORY_SEARCH_LIST) ?? [];
  }

  Future<void> addSearchHistory(String value) async {
    await AppDB.insertOrUpdateStringWithIndex(Constant.KEY_APP_HISTORY_SEARCH_LIST, value);
  }

  Future<void> removeSearchHistory(String value) async {
    await AppDB.removeStringList(Constant.KEY_APP_HISTORY_SEARCH_LIST, value);
  }
        ...
        () => ExtendedNestedScrollView(
          ...
          body: AnimatedSwitcher(
            ...
            child: showHistory.value
                ...
                : Column(children: <Widget>[Expanded(child: _buildTabBarView())]),
          ),
        ),
      ),
    );
  ...
  Widget typeItem({double? widget = 100, double? height = 100, Map data = const {}, Function(Map type)? onTap}) {
    return HyperCard(
      ...
      child: InkWell(
        ...
        child: GridTile(
          header: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8), alignment: Alignment.topLeft, child: Icon(data["icon"])),
          footer: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8), alignment: Alignment.bottomRight, child: Text(data["name"])),
          child: Container(),
        ),
      ),
    );
