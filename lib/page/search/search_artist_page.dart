...
class SearchArtistPage extends StatefulWidget {
  const SearchArtistPage({super.key, required this.plugin, required this.controller});
  ...
  @override
  SearchTabPageState<SearchArtistPage> createState() => _SearchArtistPageState();
}

class _SearchArtistPageState extends SearchTabPageState<SearchArtistPage> with AutomaticKeepAliveClientMixin {
  ...
  void searchSong({required String? keyword, int page = 0, int size = 20}) {
    ...
    ApiFactory.api(package: widget.plugin.package!)?.searchArtist(keyword: keyword ?? "", page: page, size: size).then((value) {
