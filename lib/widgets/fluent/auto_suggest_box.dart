...
typedef AutoSuggestBoxSorter<T> = List<AutoSuggestBoxItem2<T>> Function(String text, List<AutoSuggestBoxItem2<T>> items);

typedef OnChangeAutoSuggestBox<T> = void Function(String text, TextChangedReason2 reason);
typedef ItemsBuilder<T> = Future<List<AutoSuggestBoxItem2<T>>> Function(String text);

typedef AutoSuggestBoxItemBuilder<T> = Widget Function(BuildContext context, AutoSuggestBoxItem2<T> item);

enum TextChangedReason2 {
  /// Whether the text in an [AutoSuggestBox2] was changed by user input
  userInput,

  /// Whether the text in an [AutoSuggestBox2] was changed because the user
  /// chose the suggestion
  suggestionChosen,

  /// Whether the text in an [AutoSuggestBox2] was cleared by the user
  cleared,
}
...
class AutoSuggestBoxItem2<T> {
  ...
  AutoSuggestBoxItem2({required this.value, required this.label, this.child, this.onFocusChange, this.onSelected, this.semanticLabel});
...
class AutoSuggestBox2<T> extends StatefulWidget {
  ...
  @override
  State<AutoSuggestBox2<T>> createState() => AutoSuggestBox2State<T>();
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
      ...
      ..add(ObjectFlagProperty<ValueChanged<AutoSuggestBoxItem2<T>>?>('onSelected', onSelected, ifNull: 'disabled'))
      ..add(FlagProperty('clearButtonEnabled', value: clearButtonEnabled, defaultValue: true, ifFalse: 'clear button disabled'))
      ..add(FlagProperty('enableKeyboardControls', value: enableKeyboardControls, defaultValue: true, ifFalse: 'keyboard controls disabled'))
      ..add(DoubleProperty('maxPopupHeight', maxPopupHeight, defaultValue: kAutoSuggestBoxPopupMaxHeight));
  }

  /// The default item sorter.
  ///
  /// This sorter will filter the items based on their label.
  List<AutoSuggestBoxItem2<T>> defaultItemSorter(String text, List<AutoSuggestBoxItem2<T>> items) {
    text = text.trim();
...
class AutoSuggestBox2State<T> extends State<AutoSuggestBox2<T>> {
  late FocusNode _focusNode = widget.focusNode ?? FocusNode();
  OverlayEntry? _entry;
  final LayerLink _layerLink = LayerLink();
  final GlobalKey _textBoxKey = GlobalKey(debugLabel: "AutoSuggestBox's TextBox Key");

  late TextEditingController _controller;
  final FocusScopeNode _overlayNode = FocusScopeNode();
  final _focusStreamController = StreamController<int>.broadcast();
  final _dynamicItemsController = StreamController<List<AutoSuggestBoxItem2<T>>>.broadcast();
  ...
  void _updateLocalItems() {
    if (!mounted) return;
    setState(() => _localItems = sorter(_controller.text, items));
  }
  ...
  void initState() {
    ...
    _localItems = sorter(_controller.text, items);
  ...
  void _handleFocusChanged() {
    ...
    if (!hasFocus) {
      ...
      itemStream?.cancel();
      itemStream = widget.itemsBuilder?.call(_controller.text).asStream().listen((its) {
        setState(() {
          isLoading = false;
        });
        items.clear();
        items.addAll(its as Iterable<AutoSuggestBoxItem2<T>>);
        _dynamicItemsController.add(items);
        _updateLocalItems();
        showOverlay();
      });
    }
  ...
  void _insertOverlay() {
    final overlayState = Overlay.of(context, rootOverlay: true, debugRequiredFor: widget);
    ...
    _entry = OverlayEntry(
      builder: (context) {
        ...
        final globalOffset = box.localToGlobal(Offset.zero, ancestor: overlayState.context.findRenderObject());
        ...
        final maxHeight = (screenHeight - overlayY).clamp(0.0, widget.maxPopupHeight);
              ...
              child: FluentTheme(
                ...
                child: _AutoSuggestBoxOverlay<T>(
                  ...
                  onSelected: (AutoSuggestBoxItem2<T> item) {
                    ...
                    widget.onChanged?.call(item.label, TextChangedReason2.suggestionChosen);
  ...
  void _onChanged(String text) {
    widget.onChanged?.call(text, TextChangedReason2.userInput);
    dismissOverlay();
    ...
    debounceHelper.debounce(() {
      itemStream?.cancel();
      itemStream = widget.itemsBuilder?.call(text).asStream().listen((its) {
        setState(() {
          isLoading = false;
        });
        items.clear();
  ...
  void _onSubmitted() {
    ...
    final currentlySelectedIndex = _localItems.indexWhere((item) => item._selected);
    ...
    widget.onChanged?.call(_controller.text, TextChangedReason2.suggestionChosen);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final suffix = Row(
      ...
      children: [
        if (isLoading) Container(height: 20, width: 20, child: ProgressRing(strokeWidth: 2)),
        if (widget.clearButtonEnabled && _controller.text.isNotEmpty)
          Padding(
            ...
            child: IconButton(
              ...
              onPressed: () {
                _controller.clear();
                widget.onChanged?.call(_controller.text, TextChangedReason2.cleared);
              },
            ),
          ),
...
class _AutoSuggestBoxOverlayState<T> extends State<_AutoSuggestBoxOverlay<T>> {
  ...
  void initState() {
    ...
    focusSubscription = widget.focusStream.listen((index) {
      ...
      scrollController.animateTo(currentSelectedOffset, duration: const Duration(milliseconds: 100), curve: Curves.easeInOut);
      setState(() {});
    });
      ...
      child: FocusScope(
        ...
        child: Container(
          ...
          decoration: ShapeDecoration(
            ...
            shadows: [
              BoxShadow(color: Colors.black.withValues(alpha: 0.05), offset: const Offset(-1, 1), blurRadius: 2.0, spreadRadius: 3.0),
              BoxShadow(color: Colors.black.withValues(alpha: 0.05), offset: const Offset(1, 1), blurRadius: 2.0, spreadRadius: 3.0),
            ],
          ),
          child: Acrylic(
                ...
                if (sortedItems.isEmpty) {
                  result =
                      ...
                      Padding(
                        ...
                        child: _AutoSuggestBoxOverlayTile(text: Text(localizations.noResultsFoundLabel)),
                      );
                } else {
...
class _AutoSuggestBoxOverlayTile extends StatefulWidget {
  const _AutoSuggestBoxOverlayTile({required this.text, this.selected = false, this.onSelected, this.semanticLabel});
...
class __AutoSuggestBoxOverlayTileState extends State<_AutoSuggestBoxOverlayTile> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    controller = AnimationController(vsync: this, duration: const Duration(milliseconds: 125));
    controller.forward();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile.selectable(
      ...
      title: EntrancePageTransition(
        animation: Tween<double>(begin: 0.75, end: 1.0).animate(CurvedAnimation(parent: controller, curve: Curves.easeOut)),
        child: DefaultTextStyle.merge(style: theme.typography.body, child: widget.text),
      ),
