...
class FluentScaffoldPage extends StatefulWidget {
  /// Creates a new scaffold page.
  const FluentScaffoldPage({super.key, this.header, this.content = const SizedBox.expand(), this.bottomBar, this.padding, this.resizeToAvoidBottomInset = true});

  /// Creates a scrollable page
  ///
  /// The default horizontal and vertical padding is added automatically
  FluentScaffoldPage.scrollable({
    super.key,
    this.header,
    this.bottomBar,
    this.padding,
    ...
    this.resizeToAvoidBottomInset = true,
         ...
         builder: (context) {
           return ListView(
             ...
             padding:
                 padding ??
                 EdgeInsetsDirectional.only(bottom: kPageDefaultVerticalPadding, start: PageHeader.horizontalPadding(context), end: PageHeader.horizontalPadding(context)),
             children: children,
           );
         },
       );
  ...
  FluentScaffoldPage.withPadding({super.key, this.header, this.bottomBar, this.padding, required Widget content, this.resizeToAvoidBottomInset = true})
    : content = Builder(
        builder: (context) {
          return Padding(
            padding:
  ...
  @override
  State<FluentScaffoldPage> createState() => _FluentScaffoldPageState();

  @override
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties
      ..add(DiagnosticsProperty('padding', padding, defaultValue: kPageDefaultVerticalPadding))
      ..add(FlagProperty('resizeToAvoidBottomInset', value: resizeToAvoidBottomInset, defaultValue: true, ifFalse: 'do not resize'));
  }
}

class _FluentScaffoldPageState extends State<FluentScaffoldPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return PageStorage(
      bucket: _bucket,
      child: Padding(
        padding: EdgeInsetsDirectional.only(bottom: widget.resizeToAvoidBottomInset ? MediaQuery.viewInsetsOf(context).bottom : 0.0),
        child: Column(
          children: [
            Expanded(
              child: Container(
                ...
                padding: widget.padding == null ? const EdgeInsetsDirectional.only(top: kPageDefaultVerticalPadding) : EdgeInsetsDirectional.only(top: widget.padding!.top),
                child: Column(
...
/// See also:
///
///   * [FluentScaffoldPage], which creates a page that follows Windows-ui design
///                     guidelines.
...
class PageHeader extends StatelessWidget {
  ...
  const PageHeader({super.key, this.leading, this.title, this.commandBar, this.padding});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsetsDirectional.only(bottom: 18.0, start: leading != null ? 0 : horizontalPadding),
      child: Row(
        children: [
          if (leading != null) leading!,
          Expanded(
            child: DefaultTextStyle.merge(style: theme.typography.title!, child: title ?? const SizedBox()),
          ),
          ...
          if (commandBar != null) ...[
            Flexible(
              child: ConstrainedBox(
                ...
                child: Align(alignment: AlignmentDirectional.centerEnd, child: commandBar!),
              ),
            ),
            SizedBox(width: horizontalPadding),
          ],
