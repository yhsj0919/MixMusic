...
final DynamicLibrary qjsDynamicLibrary =
    Platform.isAndroid ? DynamicLibrary.open("libfastdev_quickjs_runtime.so") : (Platform.isWindows ? DynamicLibrary.open('flutter_js_plugin.dll') : DynamicLibrary.process());

typedef FnBridgeCallback = Function(dynamic args); //String Function(String channel, String message);
...
Pointer<JSValueConst> bridgeCallbackGlobalHandler(
  ...
  return QuickJsRuntime.jsEval(ctx, "'No channel registered: ($channelNameStr) => $messageStr'").rawResult;
}
...
class QuickJsRuntime extends JavascriptRuntime {
  ...
  QuickJsRuntime(this.fileName) {
    ...
    consoleLogBridgeFunction = Pointer.fromFunction<Pointer<JSValueConst> Function(Pointer<JSContext> ctx, Pointer<Utf8NullTerminated> channel, Pointer<Utf8NullTerminated> msg)>(
        bridgeCallbackGlobalHandler);
    setTimeoutBridgeFunction = Pointer.fromFunction<Pointer<JSValueConst> Function(Pointer<JSContext> ctx, Pointer<Utf8NullTerminated> channel, Pointer<Utf8NullTerminated> msg)>(
        bridgeCallbackGlobalHandler);
    sendNativeBridgeFunction = Pointer.fromFunction<Pointer<JSValueConst> Function(Pointer<JSContext> ctx, Pointer<Utf8NullTerminated> channel, Pointer<Utf8NullTerminated> msg)>(
        bridgeCallbackGlobalHandler);
  ...
  static JSEvalWrapper _jsEvalWrapper = qjsDynamicLibrary.lookupFunction<JSEvalWrapperNative, JSEvalWrapper>('JSEvalWrapper');
  ...
  static JS_NewRuntimeDartBridge _jsNewRuntimeDartBridge = qjsDynamicLibrary
      ...
      .asFunction();
  static JS_NewContextFn _jsNewContext = qjsDynamicLibrary.lookup<NativeFunction<JS_NewContextFn>>('JS_NewContextDartBridge').asFunction();

  static JS_GetNullValue _jsGetNullValue = qjsDynamicLibrary.lookup<NativeFunction<JS_GetNullValue>>('JS_GetNullValue').asFunction();

  static JSExecutePendingJob _jsExecutePendingJob = qjsDynamicLibrary.lookupFunction<JSExecutePendingJobNative, JSExecutePendingJob>(
    'JS_ExecutePendingJob',
  );

  static JSCallFunction1Arg _callJsFunction1Arg = qjsDynamicLibrary.lookupFunction<JSCallFunction1ArgNative, JSCallFunction1Arg>('callJsFunction1Arg');

  static JSGetTypeTag _jsGetTypeTag = qjsDynamicLibrary.lookupFunction<JSGetTypeTagNative, JSGetTypeTag>('getTypeTag');

  static JSIsArray _jsIsArray = qjsDynamicLibrary.lookupFunction<JSIsArrayNative, JSIsArray>('JS_IsArrayDartWrapper');

  static JSJSONStringify _jSJSONStringify = qjsDynamicLibrary.lookupFunction<JSJSONStringifyNative, JSJSONStringify>('JS_JSONStringifyDartWrapper');
  ...
  JsEvalResult callFunction(
    ...
    int operationResult = _callJsFunction1Arg(_context, function as Pointer<JSValueConst>, argument as Pointer<JSValueConst>, result as Pointer<JSValueConst>, stringResult);
  ...
  static JsEvalResult jsEval(
    ...
    _jsEvalWrapper(ctx, Utf8NullTerminated.toUtf8(js), js.length, Utf8NullTerminated.toUtf8(fileName), 0, errors, result, stringResult);
  ...
  static T? convertToValue<T>(Pointer<JSContext> context, JsEvalResult evalResult) {
